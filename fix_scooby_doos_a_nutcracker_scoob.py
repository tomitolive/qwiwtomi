...
def fix_scooby_doos_a_nutcracker_scoob_errors(file_path):
    """إصلاح أخطاء Scooby-Doos A Nutcracker Scoob في ملف JSON واحد"""
    try:
        ...
        # تصحيح "Scooby-Doos  Nutcracker Scoob" -> "Scooby-Doos A Nutcracker Scoob"
        ...
        if 'ai_content' in data:
            ...
            for field in ['desc_ar', 'opinion_ar', 'seo_title_ar', 'meta_desc', 'keywords']:
                if field in ai_content:
                    ...
                    corrected = fix_scooby_doos_a_nutcracker_scoob(original)
            ...
            # إضافة الاسم الكامل إلى keywords إذا كان ناقصاً
            if 'keywords' in ai_content:
                title_en = data.get('title_en', '')
                if title_en and title_en not in ai_content['keywords']:
                    original = ai_content['keywords']
                    corrected = f"{title_en}, {original}"
                    print(f"  إضافة الاسم الكامل إلى keywords: '{original[:50]}...' -> '{corrected[:50]}...'")
...
def main():
    ...
    for json_file in json_files:
        print(f"\n🔍 فحص {json_file.name}...")
        if fix_scooby_doos_a_nutcracker_scoob_errors(json_file):
            fixed_count += 1
