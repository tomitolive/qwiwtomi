...
def fix_el_cementerio_de_los_elefantes_errors(file_path):
    """إصلاح أخطاء El Cementerio de los Elefantes في ملف JSON واحد"""
    try:
        ...
        # تصحيح "El Cementerio   Elefantes" -> "El Cementerio de los Elefantes"
        ...
        if 'ai_content' in data:
            ...
            for field in ['desc_ar', 'opinion_ar', 'seo_title_ar', 'meta_desc', 'keywords']:
                if field in ai_content:
                    ...
                    corrected = fix_el_cementerio_de_los_elefantes(original)
...
def main():
    ...
    for json_file in json_files:
        print(f"\n🔍 فحص {json_file.name}...")
        if fix_el_cementerio_de_los_elefantes_errors(json_file):
            fixed_count += 1
